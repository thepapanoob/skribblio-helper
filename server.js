...
app.use(function(req, res, next) {
  res.setHeader('Access-Control-Allow-Origin', 'http://skribbl.io');
  res.setHeader(
...
app.post('/', (req, res) => {
  if (req.body && req.body.w)
    insertWord(req.body.w);

  if (req.body.s)
    correctWord = req.body.w;
...
app.get('/:regex', (req, res) => {
  ...
  const filterList = wordlist[req.params.regex.length] !== undefined ? wordlist[req.params.regex.length] : [];
...
function gracefulShutdown() {
  fs.writeFile(
    savePath,
    JSON.stringify(wordlist, null, 0),
    function(err) {
      if (err) {
